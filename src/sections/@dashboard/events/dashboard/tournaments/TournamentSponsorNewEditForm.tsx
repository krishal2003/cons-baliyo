...
export const re = /^(https?:\/\/)?(www\.)?[a-zA-Z0-9-]+\.[a-zA-Z]{2,}([a-zA-Z0-9._]*)*\/?$/;
...
export default function TournamentSponsorForm({
  isEdit,
  currentSponsor,
  setRefetch,
  refetch,
  ...
  const defaultValues = {
    ...
    sponsor_link: currentSponsor?.sponsor_link || '',
  ...
  const onSubmit = async (data: SponsorFormValues) => {
    ...
    if (isEdit && currentSponsor) {
      ...
      axiosInstance2
        .post('/update-tournament-sponsor/', formData, {
          headers: {
      ...
      axiosInstance2
        .post('/create-tournament-sponsor/', formData, {
          headers: {
            'Content-Type': 'multipart/form-data',
          },
        })
        ...
        <Grid item xs={12}>
          <Card sx={{ p: 3 }}>
            <Stack spacing={3}>
              ...
              <RHFTextField name="sponsor_link" label="Sponsor Link" />
