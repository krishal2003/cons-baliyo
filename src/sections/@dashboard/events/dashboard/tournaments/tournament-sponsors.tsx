...
import TournamentSponsorForm from './TournamentSponsorNewEditForm';
...
const TABLE_HEAD = [
  ...
  { id: 'link', label: 'Sponsor Link', align: 'left' },
...
export default function TournamentSponsorsPage() {
  ...
  const getFAQList = useCallback(async () => {
    try {
      const response = await axiosInstance2.get('/get-tournament-sponsors/', {
  ...
  const handleDeleteRow = (id: string) => {
    axiosInstance2
      .delete(`/delete-tournament-sponsor/`, {
        params: { id },
      })
      .then((res) => {
        console.log(res);
