...
import { Organization, Request } from 'src/@types/freePlayer';
import { useSnackbar } from 'notistack';
import {
  FreeOrganizationsTableRow,
  FreeOrganizationsTableToolbar,
  RequestedOrganizationsTableRow,
} from 'src/sections/@dashboard/freeOrganizations/list';

// ----------------------------------------------------------------------

const TABLE_HEAD = [
  { id: 'name', label: 'Organization Name', align: 'left' },
...
FreeOrganizationPage.getLayout = (page: React.ReactElement) => (
  <DashboardLayout>{page}</DashboardLayout>
);

// ----------------------------------------------------------------------

export default function FreeOrganizationPage() {
  const {
    dense,
    page,
    order,
  ...
  const getAllUsers = useCallback(async () => {
    try {
      const response = await axiosInstance2.get('/all-organizations');
      setTableData(response.data.free_organizations);
    ...
    <>
      ...
      <Container maxWidth={themeStretch ? false : 'lg'}>
        <CustomBreadcrumbs
          ...
          links={[{ name: 'Dashboard', href: PATH_DASHBOARD.root }, { name: 'Organization List' }]}
        />
      ...
      <Container maxWidth={themeStretch ? false : 'lg'} sx={{ mt: 8 }}>
        <CustomBreadcrumbs
          heading="Requested Organization List"
          links={[
            { name: 'Dashboard', href: PATH_DASHBOARD.root },
            { name: 'Requested Organization List' },
          ]}
        />
              ...
              <Table size={dense ? 'small' : 'medium'} sx={{ minWidth: 800 }}>
                ...
                <TableBody>
                  {requestedPlayers
                    .slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage)
                    .map((row) => (
                      <RequestedOrganizationsTableRow key={row.id} row={row} />
                    ))}
...
function applyFilter({
  ...
  if (filterName) {
    inputData = inputData.filter(
      (organization) =>
        `${organization.organization_name}`.toLowerCase().indexOf(filterName.toLowerCase()) !== -1
    );
  }
